...
interface IQuestionsPie {
    filter?: 'type' | 'overview'
}

ChartJS.register(ArcElement, Tooltip, Legend);

const QuestionsPie = ({ filter = 'overview' }: IQuestionsPie) => {
    ...
    const initPie = () => {
        ...
        if (filter === 'overview') {
            ...
            currentOv = [
                { label: 'answered in total', total: 0 },
                { label: 'answered correctly', total: 0 },
                { label: 'unanswered', total: 0 }
            ]
            ...
            if (assessment.choices && assessment.choices.length > 0) {
                ...
                colors = labels.map((_, index) => {
                    const bgColors = ['#6774f2', '#00bdff', '#b0cc30']
                ...
                // Generate different colors for each label
                colors = labels.map((_, index) => {
                    const bgColors = ['#DCE1F3', '#E6EBFF', '#CCD2E9']
                    return bgColors[index % bgColors.length];
                });
                    ...
                    <div className='h-full flex justify-center flex-col'>
                        ...
                        <ul className="list-none p-0">
                            {overview.map((item, index) => (
                                <li key={item.label} className="flex items-center mb-[0.45rem] gap-x-[1.3rem]">
                                    <span
                                        className="inline-block w-[9px] h-[9px] rounded-2px"
                                        style={{ backgroundColor: colors[index] }}
                                    ></span>
                                    <span className='font-mona text-[13px] goc-900'>{item.total} {helper.capitalizeWord(item.label)}</span>
                                </li>
                            ))}
                        </ul>
