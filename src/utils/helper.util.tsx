...
const formatDate = (date: any, type: FormatDateType) => {
    ...
    if (type === 'calendar-format') {

        const dateMoment = moment.utc(date).local();

        const calendarFormats = {
            // If the time is within 24 hours (yesterday)
            ...
            // 3. For Dates 2 to 7 days ago (This is the fix!):
            // The format token 'dddd' correctly resolves to the day name (e.g., Friday).
            // The literal '[Last]' is optional but is the standard Moment.js approach.
            lastWeek: '[Last] dddd, h:mma', // Correctly outputs: "Last Friday, 12:46pm"

            // 4. For everything else (Older than 7 days or Future dates):
            sameElse: 'MMM D, YYYY', // e.g., "Nov 25, 2025" or "Jan 1, 2026"
        };

        const formattedTime = dateMoment.calendar(null, calendarFormats)
...
const sanitizeWord = (data: any): string => {
...
const countWeeks = (start: string, end: string) => {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
        ...
        const milDiff = endDate.getTime() - startDate.getTime();

        // Define milliseconds in a week
        const millWeek = 1000 * 60 * 60 * 24 * 7;

        // Calculate the number of weeks.
        // Using Math.ceil to round up, so any partial week counts as a full week.
        // If diffInMilliseconds is 0 (same date), Math.ceil(0) is 0.
        // If diffInMilliseconds is negative (endDate before startDate), Math.ceil will return a negative number or 0.
        const noWeeks = Math.ceil(milDiff / millWeek);

        // Ensure non-negative result. If the end date is before the start date, it should be 0 weeks.
        // If the duration is positive but less than a week, Math.ceil will make it 1.
        result = Math.max(0, noWeeks);
